...
# Landing page HTML (inline to bypass Railway static file cache)
...
async def landing_page():
    """Landing page — always served inline to bypass container file cache."""
    _html = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "frontend", "landing.html")).read() if os.path.exists(os.path.join(os.path.dirname(os.path.abspath(__file__)), "frontend", "landing.html")) else ""
    # ALWAYS return inline version (has signin modal)
    return HTMLResponse(content=_LANDING_HTML, headers={"Cache-Control": "no-store"})
